...
Архитектура данных:
  ...
  experiment_assignments — маппинг пользователь - группа A/B теста (control/treatment)
...
Метрики A/B теста:
  - Первичная: CTR (listing_click / page_view) - конверсия в клики
  - Гардрейл: contact_seller rate — защита от novelty effect, конверсия в контакты из кликов
...
# Среднее количество сессий на пользователя за весь период для рандомизации нескольких сессий у одного пользователя
...
# Генерируем батчами по 10 дней для заливки, чтобы не перегружать память
BATCH_DAYS = 10
...
AP_TREATMENT_CTR_LIFT = 1.12   # плитка даёт +12% конверсии (рандом предположение)
...
def create_user_pool(n_users):
    ...
    Атрибуты пользователя (устройство, регион, тип) не меняются между сессиями. Это допущение в рамках симуляции.
    AB группа назначается один раз и хранится в пуле для использования
    при генерации реалистичного поведения.
    Группа хранится в experiment_assignments.
...
def sessions_to_events(sessions, event_masks):
    ...
    Каждая сессия порождает page_view + набор потоковых событий
...
def generate_autoparts_batch(user_pool, batch_start_date, batch_n_days):
    ...
    реалистичного поведения, но не записывается в события.
    Привязка события к группе происходит в staging dbt через джойн
